...
const ResidentialUpdate = () => {
  ...
  const {mutate, isLoading} = useMutation(
    async (payload: Payload) => await ApiService.secure().post("/residential/edit", payload),
    {
      onSettled: (response) =>
  ...
  useEffect(() => {
    const getDetails = async () => {
      try {
        const response = await ApiService.secure().get(`/residential/detail/${id}`);
        if (response.data.status !== 200) {
          throw new Error(response.data.message);
        }

        const result: ResidentialItem = response.data.data;
        form.reset({
          top_name: result.top_name,
          meta_description: result.meta_description,
          meta_title: result.meta_title,
          background: [result.background._id],
          logo: [result.logo._id],
          section2_description: result.section2_description,
          section2_list: result.section2_list || [],
          top_description: result.top_description,
          title: result.title,
          active_status: result.active_status,
          section2_image: [result.section2_image._id],
          thumbnail: [result.thumbnail._id],
          content: result.content.map((item) => ({
            image: item.image.map((img) => img._id),
            link: item.link,
            name: item.name,
          })),
          section1: result.section1.map((item) => ({
